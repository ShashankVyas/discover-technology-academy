...
async function run() {
	...
	while (true) {
		...
		if (verificationResult === 'ProofValidated') {
			console.log('Recipient is validated!')
			await wsSend({ type: "log", data: "Recipient is validated" })
		} else {
			console.log('Recipient is NOT validated')
			await wsSend({ type: "log", data: "Recipient is NOT validated" })
		}
	}
}
